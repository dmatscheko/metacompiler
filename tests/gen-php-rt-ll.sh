#!/usr/bin/env bash
#
# gen-php-rt-ll.sh - compile languages/lib/php-rt.metajs with OUR OWN MetaJS
# compiler and keep the module as languages/lib/php-rt.ll.
#
# This is layer 2 of docs/runtime-next-plan.md part 3 for PHP: PHP's own
# semantics, written in MetaJS, compiled by languages/metajs-to-llvm-ir.abnf with
# -rt-lib into a module that php-to-llvm-ir.abnf links NEXT TO lib/runtime.ll (the
...
#
# php-rt.ll is a generated artifact like abnf/jsbootstrap.ll and
# languages/lib/runtime.ll: regenerate it whenever php-rt.metajs or
...
#
# Usage: tests/gen-php-rt-ll.sh [--check]
...
"$BIN" languages/metajs-to-llvm-ir.abnf languages/lib/php-rt.metajs -q -rt-lib \
    | module_only > "$OUT" || { echo "gen-php-rt-ll.sh: compile failed" >&2; exit 2; }

lines="$(wc -l < "$OUT" | tr -d ' ')"
[ "$lines" -gt 100 ] || { echo "gen-php-rt-ll.sh: only $lines lines - the compile must have failed" >&2; exit 2; }

if [ "$CHECK" -eq 1 ]; then
    if diff -q "$OUT" languages/lib/php-rt.ll >/dev/null 2>&1; then
        echo "php-rt.ll is up to date ($lines lines)"
        exit 0
    fi
    echo "php-rt.ll is STALE - rerun tests/gen-php-rt-ll.sh" >&2
    diff "$OUT" languages/lib/php-rt.ll | head -20 >&2
    exit 1
fi

cp "$OUT" languages/lib/php-rt.ll
echo "wrote languages/lib/php-rt.ll ($lines lines)"
