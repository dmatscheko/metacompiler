// Fast feature-matrix test for the TypeScript interpreter (typescript-interpreter.abnf)
// and the LLVM-IR compiler (typescript-to-llvm-ir.abnf). It replaces the four
// algorithm-themed typescript-test-big-* stress tests: instead of large loops every
// implemented construct is exercised with the SMALLEST program that can prove it
// works - loops run 0, 1 or 3 times, recursion stays below depth 6. Types are erased
// but annotated everywhere so the type surface parses. A failed check prints its id
// and main() returns the failure count; exit 0 and byte-identical output on all four
// legs (interpreter/compiler x goja/-frozen) mean everything passed.
...
let lazyReached: number = -1;
function* lazyCounter(): Generator<number, void, void> { let i: number = 0; while (i < 100) { lazyReached = i; yield i; i = i + 1; } }
let lazyTaken: number[] = [];
...
let lazyCursor = { i: 0, next: function(): any { this.i = this.i + 1; if (this.i > 3) { return {value: undefined, done: true}; } return {value: this.i * 10, done: false}; } };
let lazyOut: number[] = [];
...
lazyReached = -1;
function* lazyDeleg(): Generator<number, void, void> { yield* lazyCounter(); }
let lazyDn: number[] = [];
...
// ----- TypeScript-only surface: interfaces, type aliases, enums, typed classes -----
interface Named {
    name: string;
    tag?: string;              // optional member
}
type Pair = { a: number, b: number };
function takeNamed(n: Named): string { return n.name; }
check("ts-interface-erased", takeNamed({ name: "iface" }) === "iface");
function addPair(p: Pair): number { return p.a + p.b; }
check("ts-type-alias", addPair({ a: 2, b: 3 }) === 5);

enum Color { Red, Green, Blue }
enum Start { A = 5, B }
check("ts-enum-auto", Color.Red === 0 && Color.Green === 1 && Color.Blue === 2);
check("ts-enum-start", Start.A === 5 && Start.B === 6);
...
check("ts-typed-class", new TypedBox(7, "n").describe() === "n=7");

function identity<T>(x: T): T { return x; }
check("ts-generic-fn", identity(4) === 4 && identity("g") === "g");

let annotated: string = "typed";
let arrTyped: number[] = [1, 2, 3];
check("ts-annotations", annotated === "typed" && arrTyped[2] === 3);
...
const qlog: string[] = [];
async function qstep(tag: string): Promise<void> { await 0; qlog.push(tag); }
async function qguard(tag: string): Promise<void> { try { await Promise.reject("r"); } catch (e) { qlog.push(tag + e); } }
...
qstep("a");
Promise.resolve(5).then(function (v: any) { qlog.push("v" + v); });
qguard("g");
Promise.all([1, Promise.resolve(2)]).then(function (a: any) { qlog.push("all" + a.join("")); });
...
Promise.resolve().then(qnop).then(qnop).then(qnop).then(qnop).then(function () {
    const ok: boolean = qlog.join(",") === "z,t1,a,v5,gr,t2,all12";
