#!/usr/bin/env bash
#
# gen-go-rt-ll.sh - compile languages/lib/go-rt.metajs with OUR OWN MetaJS
# compiler and keep the module as languages/lib/go-rt.ll.
#
# This is layer 2 of docs/runtime-next-plan.md part 3 for Go: Go's own
...
#
# go-rt.ll is a generated artifact like abnf/jsbootstrap.ll and
# languages/lib/runtime.ll: regenerate it whenever go-rt.metajs or
...
#
# Usage: tests/gen-go-rt-ll.sh [--check]
...
if [ -n "$MEC_BIN_ABS" ]; then
    BIN="$MEC_BIN_ABS"
    OUT="$(mktemp -t mec-gort.XXXXXX)"
    trap 'rm -f "$OUT"' EXIT
else
    BIN="$(mktemp -t mec-gengo.XXXXXX)"
    OUT="$(mktemp -t mec-gort.XXXXXX)"
    trap 'rm -f "$BIN" "$OUT"' EXIT
    go build -o "$BIN" . || { echo "gen-go-rt-ll.sh: build failed" >&2; exit 2; }
fi
...
"$BIN" languages/metajs-to-llvm-ir.abnf languages/lib/go-rt.metajs -q -rt-lib \
    | module_only > "$OUT" || { echo "gen-go-rt-ll.sh: compile failed" >&2; exit 2; }

lines="$(wc -l < "$OUT" | tr -d ' ')"
[ "$lines" -gt 100 ] || { echo "gen-go-rt-ll.sh: only $lines lines - the compile must have failed" >&2; exit 2; }

if [ "$CHECK" -eq 1 ]; then
    if diff -q "$OUT" languages/lib/go-rt.ll >/dev/null 2>&1; then
        echo "go-rt.ll is up to date ($lines lines)"
        exit 0
    fi
    echo "go-rt.ll is STALE - rerun tests/gen-go-rt-ll.sh" >&2
    diff "$OUT" languages/lib/go-rt.ll | head -20 >&2
    exit 1
fi

cp "$OUT" languages/lib/go-rt.ll
echo "wrote languages/lib/go-rt.ll ($lines lines)"
