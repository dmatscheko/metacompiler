...
Backslash unescapes a string. Necessary for tokens. It is about the inverse of `printf("%q", s)` but without the quotation marks.
* __unescapeTilde(s string) string__  
Backslash unescapes a `Code` string. Necessary for tags `Code` strings. It basically only unescapes a `\~` into a `~` (tilde) and leaves everything else untouched.
