...
The only hirarchy or grouping inside the ASG is done by the `Tag` rules. The found strings (`Token`) of the processed target text are stored as flat `Sequences` of strings below its responsible `Tag`. In other words, each `Tag` can contain multiple other `Tags` and also `Token`. So the hirarchy of `Tags` is not flat.
