...
func simplifyObj(obj interface{}, mustBeGroup ...bool ) object { // mustBeGroup = false
...
func XXXX_simplifyObj(production seq.Sequence) seq.Sequence {
	if production.Operator == seq.Group || production.Operator == seq.Basic {
		if len(production.Objects) == 1 {
			if prod, ok := production.Objects[0].(seq.Sequence); ok {
				return prod
			}
		} else if len(production.Objects) > 1 {
			newProduction := seq.Sequence{}
			newProduction.Operator = production.Operator
			for _, prod := range production.Objects {
				if prod1, ok := prod.(seq.Sequence); ok {
					newProduction.Objects = append(newProduction.Objects, simplifyObj(prod1))
				} else if prods, ok := prod.([]interface{}); ok {
					for _, pr := range prods {
						newProduction.Objects = append(newProduction.Objects, pr)
					}
				} else {
					newProduction.Objects = append(newProduction.Objects, prod)
				}
			}
...
func (co *compiler) handleTagCode(code string, childMatchStrJoined string, childCodeResultStrJoined string, childCodeResultObjTree []seq.Sequence, localVars map[string]object, localTree object) (string, []seq.Sequence) { // => (codeResultStrJoined, codeResultObjTree) // TODO: the result should be an object. write a serializer for the end result. maybe it needs multiple passes. For example to be able to call functions that are defined by the EBNF. A good place for functions is e.g. the preamble.J
	...
	funcMap := template.FuncMap{
		...
		"sLen": func(o interface{}) int {
			if o == nil {
				return 0
			}

			if q, ok := o.(seq.Sequence); ok {
				return len(q.Objects)
			}

			// if seq, ok := o.([]interface{}); ok {
			// 	if len(seq) > 0 {
			// 		if _, ok := seq[0].(oldGroup); ok {
			// 			return len(seq) - 1
			// 		}
			// 	}
			// 	return len(seq)
		...
		"upstream": func(args ...interface{}) string {
			...
			// if a sequence is fully nil, return nil
			allNil := true
			for _, o := range args {
				if o != nil {
					allNil = false
			...
			for _, arg := range args {
				if a, ok := arg.([]seq.Sequence); ok {
					res = append(res, a...)
				}
			}

			TODO: complete!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			upstream should convert into the corrent object form

			// if a, ok := args.([]seq.Sequence); ok {
			// 	codeResultObjTree = a
			// }
			return ""
		},
		...
		"group": func(args ...interface{}) object {
			...
			return seq.Sequence{Operator: seq.Group, Objects: args}
		},

		"fixedgroup": func(args ...interface{}) object {
			// if a sequence is fully nil, return nil
			allNil := true
			for _, o := range args {
				if o != nil {
			...
			return seq.Sequence{Operator: seq.Fixed, Objects: args}
		},
		...
		"addProduction": func(ident string, idx int, production object) string {
			co.newGrammar.productions = append(co.newGrammar.productions, seq.Sequence{Operator: seq.Production, String: ident, Int: idx, Objects: []seq.Object{production}})
			return ""
		},
...
func (co *compiler) compile(productions []seq.Sequence, inLocalVars map[string]object) (string, string, []seq.Sequence, map[string]object) { // => (childMatchStrJoined, codeResultStrJoined, codeResultObjTree, outLocalVars)
	...
	if len(productions) > 1 { // "SEQUENCE" Iterate through all rules and apply.
		matchStr := ""
		codeResultStr := ""
		codeResultObjTree := []seq.Sequence{}
		...
		for _, rule := range productions { // TODO: IMPORTANT!!! Optimize this with index to the specific production/rule, like in the grammarparser.go. And also implement a feature to state the starting rule!
			childMatchStr, childCodeResultStr, childCodeResultObjTree, childLocalVars := co.compile([]seq.Sequence{rule}, inLocalVars)
	...
	case seq.Tag:
		...
		childMatchStrJoined, childCodeResultStrJoined, childCodeResultObjTree, childLocalVars := co.compile(rule.Childs, inLocalVars) // evaluate the child productions of the TAG
		...
		codeResultStrJoined, codeResultObjTree := co.handleTagCode(tagCode, childMatchStrJoined, childCodeResultStrJoined, childCodeResultObjTree, outLocalVars, productions)
...
func CompileParseTree(productions []seq.Sequence, traceEnabled bool) (g Grammar, err error) {
	// defer func() {
	...
	childMatchStrJoined, codeResultStrJoined, codeResultObjTree, outLocalVars := co.compile(productions, localVars)
