...
// TODO: rename package in cmd, rename Sequence in Command
...
type Sequence struct {
	...
	Childs    []Sequence // Used by most Operators
	TagChilds []Sequence // Only used when Operator == seq.Tag
}
...
type Sequence_2 struct {
	Operator OperatorID
	Objects  []Object
}

// Appends a Sequence but dissolves basic SEQUENCE groups
func AppendPossibleSequence(target []Sequence, source Sequence) []Sequence {
	if source.Operator == Basic {
...
// Appends a Sequence but dissolves basic SEQUENCE groups
func AppendArrayOfPossibleSequences(target []Sequence, source []Sequence) []Sequence {
